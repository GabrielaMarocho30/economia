        ...
        if(isset($_POST['prestamo']) && isset($_POST['tiempo']) && isset($_POST['tasa'])){
            ...
            for ($i=0; $i<=$tiempo; $i++){
                ...
                if ($i == 0) {
                    ...
                    $mensualidad = $interes + $amortizacion; // calcular la mensualidad del periodo
                ...
                if ($i == $tiempo) {
                    $amortizacion = $prestamo; // si es el último periodo, la amortización es igual al monto del préstamo
                    $mensualidad = $prestamo + $interes;
                } else {
                    $amortizacion = $saldo_anterior * $tasa; // calcular la amortización del periodo
                }
            }
            echo "</table>";
        }
